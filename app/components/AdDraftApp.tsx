      ...
      <div className="col-span-7 border rounded-lg bg-white shadow-sm p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="relative border-2 border-gray-200 rounded-lg overflow-hidden hover:border-blue-400 transition-colors">
            ...
            <div className="aspect-square bg-linear-to-br from-blue-100 to-green-100 flex items-center justify-center">
              <div className="text-4xl">🖼️</div>
            </div>
            <div className="absolute bottom-1.5 right-1.5 flex gap-1">
              <button className="w-6 h-6 bg-white/90 rounded-full flex items-center justify-center hover:bg-white text-xs">
...
const TableView = () => {
  ...
  const initializeTableRows = useAdStore((state) => state.initializeTableRows);

  useEffect(() => {
    if (tableRows.length === 0) {
      console.log("[Table] 📥 Initializing rows");
      initializeTableRows();
    }
  }, [tableRows.length, initializeTableRows]);

  // Row-specific handler that marks as customized
    ...
    <div className="border rounded-lg bg-white shadow-sm">
                ...
                <tr key={row.id} className="border-b hover:bg-gray-50">
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative w-16 h-16 rounded overflow-hidden bg-linear-to-br from-purple-100 to-pink-100 flex items-center justify-center">
                        <span className="text-2xl">🖼️</span>
                        <div className="absolute top-1 left-1 bg-green-500 text-white text-xs px-1.5 py-0.5 rounded">
                          SINGLE
                        </div>
                        {row.isCustomized && (
                          <div className="absolute bottom-1 right-1 bg-orange-500 text-white text-xs px-1.5 py-0.5 rounded">
                            ✏️
                          </div>
                        )}
                      </div>
                  ...
                  <td className="p-2">
                    <input
                      ...
                      value={row.headline}
                      onChange={(e) =>
                        handleRowChange(row.id, "headline", e.target.value)
                      }
                      className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Enter headline..."
                    />
                  ...
                  <td className="p-2">
                    <select
                      ...
                      onChange={(e) =>
                        handleRowChange(row.id, "callToAction", e.target.value)
                      }
                      className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                      <option value="">Select...</option>
                  ...
                  <td className="p-2">
                    <select
                      value={row.launchAs}
                      onChange={(e) =>
                        handleRowChange(row.id, "launchAs", e.target.value)
                      }
                      className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                      <option value="active">Active</option>
      ...
      <div className="border-t bg-gray-50 p-3 flex items-center justify-between">
        ...
        <div className="text-xs text-yellow-600 bg-yellow-50 px-3 py-2 rounded">
          <strong>⚠️ Test:</strong> Make any row unique, then switch to Gallery
          Mode and back.
        </div>
      </div>
    </div>
